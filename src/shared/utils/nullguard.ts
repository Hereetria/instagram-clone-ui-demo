...
const getErrorDetails = (error: Error): errorResponse => {
...
const checkNull = (element: any, elementName: string): boolean => {
  const error: Error = Error();
  const response: errorResponse =  getErrorDetails(error);
